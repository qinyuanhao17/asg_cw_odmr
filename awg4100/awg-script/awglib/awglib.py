...
class WAVE:
    ...
    def set_length(self, length):
        if self.type != 'delay' and (length < _LENGTH_MIN):
            add_warning_info('WAVE length should in ragne [{}, {}]'.format(_LENGTH_MIN, _LENGTH_MAX))
        if  length > _LENGTH_MAX:
            raise ValueError('WAVE length should in ragne [{}, {}]'.format(_LENGTH_MIN, _LENGTH_MAX))
        self._length = length
    ...
    def __call__(self, loop, mode=None):
        """ 用元组表示一个波形的循环 """
        if loop > 4294967296:
            raise ValueError('paras size should in ragne [{}, {}]'.format(0, 4294967296))
    ...
    def search_corr_obj(self):
        ...
        while len(stack) != 0:
            ...
            if curr_w.corr_obj != None:
                _wd, _md = curr_w.corr_obj[0].search_corr_obj()
                stack.extend(_wd.values())
                if isinstance(curr_w.corr_obj[1], WAVE):
                    _wd, _md = curr_w.corr_obj[1].search_corr_obj()
                    stack.extend(_wd.values())
...
f=None, A=None, L=None, ph0=None, b=None):
    ...
    if Loop > _LENGTH_MAX:
        raise ValueError("Sin() Loop should in ragne [{}, {}]".format(_LENGTH_MIN, _LENGTH_MAX))
        
    Len = int(Len)
    if Len*Loop < _LENGTH_MIN:
        add_warning_info("Sin() length should in ragne [{}, {}]".format(_LENGTH_MIN, _LENGTH_MAX))
    if Len*Loop > _LENGTH_MAX:
        raise ValueError("Sin() length should in ragne [{}, {}]".format(_LENGTH_MIN, _LENGTH_MAX))
...
def Gauss(Var=None, Amp=None, Len=None, Offset=0, Loop=1, var=None, A=None, L=None, b=None):
    ...
    if Len*Loop > _LENGTH_MAX:
        raise ValueError("Gauss length should in ragne [{}, {}]".format(_LENGTH_MIN, _LENGTH_MAX))
    if Offset is None:
        if b is None:
            Offset = 0
        else:
...
def Square_p(Len, Mode=UP, Amp=None, Offset=0, Loop=1, Dutycycle=0.5):
    ...
    if Len * Loop < _LENGTH_MIN:
        add_warning_info("Square Len * Loop should in range [{}, {}]".format(_LENGTH_MIN, _LENGTH_MAX))
    if Len * Loop > _LENGTH_MAX:
        raise ValueError("Square Len * Loop should in range [{}, {}]".format(_LENGTH_MIN, _LENGTH_MAX))
...
def Triangle_p(Len, Mode=UP, Amp=None, Offset=0, Loop=1, Symmetry=0.5):
    global GAmp
    if Amp == None:
        Amp = GAmp
    if Mode != UP and Mode != DOWN:
    ...
    if Len * Loop < _LENGTH_MIN:
        add_warning_info("Triangle Len * Loop should in range [{}, {}]".format(_LENGTH_MIN, _LENGTH_MAX))
    if  Len * Loop > _LENGTH_MAX:
        raise ValueError("Triangle Len * Loop should in range [{}, {}]".format(_LENGTH_MIN, _LENGTH_MAX))
    if abs(Amp + Offset) > _VOLTE_RANGE:
    ...
    ret = WAVE('triangle')
    ret.params = [Len, Amp, Mode, Offset, Loop, Symmetry]
    ret.set_length(Len * Loop)
    return ret

def Triangle(T, mode=UP, A=None, b=0, N=1, Symmetry=0.5):
    L = T * SR
    return Triangle_p(L, mode, A, 0, N, Symmetry)

def Sweep(Fstart, Fstop, Stime, unit):
    if Fstart < 0 or Fstart > 330:
        raise ValueError("Sweep() Fstart must in ragne [0, 330]MHz")
    if Fstop < 0 or Fstop > 330:
        raise ValueError("Sweep() Fstop must in range [0, 330]MHz")
    if Fstop <= Fstart:
        raise ValueError("Sweep() Fstop must be greater than Fstart")
    
    u2v = {'ns':1, 'us':1000, 'ms':1000000}
    if unit not in u2v.keys():
        raise ValueError("Sweep() unknown unit {}".format(unit))
        
    wlen = int(Stime * u2v[unit] * SR)
    if wlen < _LENGTH_MIN or wlen > _LENGTH_MAX:
        raise ValueError("Sweep() Stime must be in range [{:.0f}ns, 445ms] greater than 0".format(_LENGTH_MIN / SR))
    
    ret = WAVE('fsweep')
    ret.params = [Fstart, Fstop, Stime]
    ret.set_length(wlen)
    return ret
...
class ADVS:
    ...
    def __init__(self, obj=None, repeat=1):
        ...
        if obj is None:
            ...
            raise TypeError('can\’t construct a ADVS by {}'.format(type(obj)))
