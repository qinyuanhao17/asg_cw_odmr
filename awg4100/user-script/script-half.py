...
_WAVE_ALLOWED_TYPE = ['sin', 'sinc', 'gauss', 'square', 'triangle', 'mul', 'add']
_OUT_DATA_ = False
_OUT_SEG_ = False
...
def cache_wave_file(data, mode='volt'):
    global CACHE_NUMBER
    tdir = os.environ['TEMP']
    filepath = os.path.join(tdir, 'wave-cache-{:015}'.format(CACHE_NUMBER))
    CACHE_NUMBER += 1
    if mode == 'volt':
        data = [_Encode(d) for d in data]
...
class WAVE:
        ...
        if isinstance(wave, list) and isinstance(wave[0], (int, float)):
            ...
            if len(wave) > 5 and wave[-5:] == '.wave':
                ...
                if right_path:
                    ...
                    self.length = int(file_info.st_size/2)
    ...
    def __repr__(self):
        ...
        if self.type in ['sin', 'sinc', 'gauss', 'square', 'triangle']:
            ...
            log_and_exit("unknow WAVE")
        return ret


class SubSegment:
    def __init__(self, obj, loop=1, mode=C):
        if isinstance(obj, WAVE) and isinstance(loop, int) and loop > 0 and mode in _MODE_CLASS:
            self.w = obj
            self.l = loop
            self.m = mode
        else:
            log_and_exit('Error: paramer error, when construct a SEQ')
...
class Segment:
    ...
    def __init__(self, sslist, dtime):
        ...
        if isinstance(sslist, list) and isinstance(sslist[0], SubSegment):
            ...
            log_and_exit("Type error! 0x1000")
